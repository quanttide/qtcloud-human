#!/bin/bash
# 为 human.quanttide.com 配置 CDN HTTPS 证书 + DNS CNAME。
#
# 背景：human.quanttide.com 是量潮人事云工作台发布域名，CDN 源站为 OSS 桶 qtcloud-human-studio
...
# 需在结果中精确过滤 RR=human 且 Type=CNAME
EXISTING=$(aliyun alidns DescribeDomainRecords --DomainName quanttide.com --RRKeyWord human --ValueKeyWord "${DOMAIN}.w.kunlunaq.com" 2>/dev/null | python3 -c "
import sys, json
try:
    recs = json.load(sys.stdin).get('DomainRecords', {}).get('Record', [])
    cnames = [r for r in recs if r.get('RR') == 'human' and r.get('Type') == 'CNAME']
    print(len(cnames))
except Exception:
    print(0)
")
